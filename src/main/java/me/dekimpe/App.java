...
public class App 
{
    public static void main( String[] args )
    {
        ...
        Dataset<Row> joined = pagelinks.join(revisions, revisions.col("id"), "left_outer").filter("title = '" + subject + "'");//.where((pagelinks.col("title").equalTo(subject)).or(revisions.col("title").equalTo(subject))).cache();
